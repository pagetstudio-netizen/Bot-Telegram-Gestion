...
function buildSettingsKeyboard(group: any) {
  return {
    inline_keyboard: [
      [
        {
          text: group.isActive ? "🟢 Bot ACTIF — Cliquer pour désactiver" : "🔴 Bot INACTIF — Cliquer pour activer",
      ...
      [
        {
          text: `⚠️ Max avert. : ${group.maxWarnings}`,
          callback_data: `change:maxWarnings:${group.telegramId}`,
        },
        {
          text: `🔇 Mute : ${group.muteDuration / 60} min`,
      ...
      [
        {
          ...
          callback_data: `change:floodLimit:${group.telegramId}`,
        },
      ],
      [
        {
...
function buildSettingsText(group: any) {
  const statusLine = group.isActive
    ...
    : "🔴 *Statut : INACTIF* — La modération est en pause.\n_Activez le bot pour commencer la modération._";
  ...
  return (
    ...
    `• Anti-Flood : ${group.antiFlood ? "✅" : "❌"}\n` +
    `• Anti-Liens : ${group.antiLinks ? "✅" : "❌"}\n` +
    `• Anti-Grossièretés : ${group.antiProfanity ? "✅" : "❌"}\n\n` +
    `📊 *Limites :*\n` +
...
export function setupCommands(bot: Telegraf) {
  ...
  bot.command("setrules", async (ctx) => {
    ...
    await ctx.reply("✅ *Règles du groupe mises à jour !* Tapez /rules pour les afficher.", {
      parse_mode: "Markdown",
    });
  });
  ...
  // ─── Callback query (boutons inline settings) ─────────────────────────────
  ...
  bot.command("warn", async (ctx) => {
    if (ctx.chat.type === "private") return;
    if (!(await isAdmin(ctx))) return ctx.reply("❌ Seuls les administrateurs peuvent utiliser cette commande.");
    ...
    if (Number(totalWarns) >= maxWarnings) {
      try {
        ...
        await ctx.reply(`🔨 *${target.first_name}* a été banni automatiquement après ${maxWarnings} avertissements.`, { parse_mode: "Markdown" });
  ...
  bot.command("mute", async (ctx) => {
    ...
    try {
      ...
      await ctx.reply(`🔇 *${target.first_name}* est muet pour *${minutes} minutes*.`, { parse_mode: "Markdown" });
    } catch (err) {
  ...
  bot.command("warnings", async (ctx) => {
    ...
    if (warns.length === 0) {
      ...
      const list = warns.slice(-5)
        .map((w, i) => `${i + 1}. ${w.reason ?? "Sans raison"} (${new Date(w.createdAt).toLocaleDateString("fr-FR")})`)
        .join("\n");
      await ctx.reply(`⚠️ *Avertissements de ${target.first_name}* : ${warns.length}/${maxWarnings}\n\n${list}`, { parse_mode: "Markdown" });
    }
  });
